...
const platformNav = [
  { group: '平台运营', items: [{ id: 'overview', label: '平台总览', icon: 'grid' }, { id: 'organizations', label: '机构与租户', icon: 'building' }] },
...
function SettingsPanel({ onNotify }) { return <div className="settings-panel"><SectionHeader title="平台全局配置" subtitle="变更将影响所有租户，提交后会写入审计日志" /><div className="settings-form"><label>默认会话保留时长<select><option>180 天</option><option>365 天</option></select></label><label>全局 AI 安全模式<select><option>严格模式（推荐）</option><option>标准模式</option></select></label><label>默认首响 SLA<input defaultValue="60" /></label><label>通知邮箱<input defaultValue="ops@ai-service.demo" /></label></div><button className="admin-primary-btn" onClick={() => onNotify('全局配置已保存（演示）')}>保存配置</button></div> }
...
function MetricGrid({ stats }) { return <div className="admin-metric-grid">{stats.map((stat) => <div className={`admin-metric-card ${stat.tone}`} key={stat.label}><div className="metric-icon"><AIcon name={stat.icon} size={18}/></div><span>{stat.label}</span><strong>{stat.value}</strong><small>{stat.delta}</small></div>)}</div> }
...
function DataTable({ headers, rows }) { return <div className="table-wrap"><table className="admin-table"><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index}>{row.map((cell, cellIndex) => <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table></div> }
